...
import * as services from "services/x-publish/course/coursePub"
				...
				.maindata
					.serachWrap
						...
						form#mainForm.clearfix
							span.group-inline
								el-input(placeholder="共享课程名称" name="name" clearable v-model="searchQuery.name")
							span.group-inline
								...
								tr(v-for="item in list" :key='item.id')
									...
									td
										.btns
											...
											Icon(size="mini" title="审核"  @click="saveExamine('创建新版本',[item.id])" v-if="item.audits_status != 1"  type="icon-menu-publish"  plain  )
