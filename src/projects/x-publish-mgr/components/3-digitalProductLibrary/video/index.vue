    ...
    delChoosens(id) {
      ...
      if(ids.length > 0){
        this.$confirm("是否删除?").then(res=>{
          ...
          this.services.del({id:ids}).then(_ => {
...
.index.clearfix
	...
	Menu(curr="/digitalProductLibrary/video")
	.mainRight
		.contentWrap
			.contentLeft
				.maindata
					.serachWrap
						...
						form#mainForm.clearfix
							span.group-inline
								el-input(placeholder="视读名称" name="name" clearable  v-model="searchQuery.name")
							span.group-inline
						...
						table.table
							...
							tbody(v-if="list")
								tr(v-for="item in list" :key='item.id')
									...
									td {{item.count}}
									td(:class="getColor(item.status)") {{item.status=="审核未通过"?"未通过":item.status}}
