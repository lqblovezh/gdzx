...
import * as services from "services/x-publish/product"
...
export default {
  ...
  methods: {
    ...
    urgentDigitalBook() { //加急审核
      ...
      if (ids) {
        this.services.urgentDigitalBook({
          id: ids
...
.index.clearfix
	...
	Menu(curr="/digitalProductLibrary/release")
	.mainRight
		.contentWrap
			.contentLeft
				.maindata
					.serachWrap
						...
						form#mainForm.clearfix
							...
							span.group-inline
								el-input(placeholder="ISBN" name="book_isbn" clearable v-model="searchQuery.book_isbn")
							span.group-inline
								el-input(placeholder="作者" name="author" clearable v-model="searchQuery.author")
							span.group-inline
								select.form-control( placeholder="分发平台" name="platforms" style="width:110px")
									option(value="") 分发平台
									option(value="x_read") XRead
									option(value="ecommerce" v-if="$config.isShowRetailers") 电商平台
							span.group-inline
						...
						table.table
							...
							tbody(v-if="list")
								tr(v-for="item in list" :key='item.id')
									td
										input(type="checkbox"  :value="item.id" name="ids" ref="ids" :data-type="item.status")
									td
										img.cover-photo(:src="item.img_src")
										.th-name.bookName(:title="item.name") {{item.name}}
									td
										.th-name(:title="item.book_isbn") {{item.book_isbn}}
									td
										.th-name(:title="item.author") {{item.author}}
									td
										ul.contList
											li(v-for="items,key in item.platforms" v-if="item.platforms&&key=='retailer'?$config.isShowRetailers:true")
												span(v-text="key=='retailer'?'电商平台':key" :class="{disabled:items==0}")
