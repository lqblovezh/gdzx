...
.index.clearfix
	...
	Menu(curr="/digitalProductLibrary/audio")
	.mainRight
		.contentWrap
			.contentLeft
				.maindata
					.serachWrap
						...
						form#mainForm.clearfix
							span.group-inline
								el-input(placeholder="听读名称" name="name" clearable v-model="searchQuery.name")
							span.group-inline
						...
						table.table
							...
							tbody(v-if="list")
								tr(v-for="item in list" :key='item.id')
									td
										input(type="checkbox"  :value="item.id" name="ids" :data-type="item.status" ref="ids")
									td
