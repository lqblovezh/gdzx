...
import AnchorList from  '../theme/anchorList'
...
export default {
  components: {VaForm ,AnchorList, Reader},
  ...
  computed:{
    isBook() {
      return this.anchorType == Anchors_chapter || this.anchorType == Anchors_content
    }
  },
  methods: {
    async getGlist(){//获取分组list
      if(!this.detailData.id) return
      await nowledgServices.listGroup({id:this.detailData.id}).then(res => {
    ...
    groupDel(item){// 删除分组
      this.$confirm("是否删除该知识结构?").then(res=>{
        nowledgServices.deleteGroup({id:[item.id],type:this.detailData.type_number,pid:this.detailData.id}).then(res=>{
          this.getGlist()
          this.$emit('saveok')
        }).catch(res=>{
          for(let i in payload.messages){
    ...
    async groupCommit(){//知识点解构新增修改
      if (!this.$refs.groupFormVa.valiAll()) return
      var query = formSerialize(this.$refs.groupForm, {hash: true})
      try {
        if(!query.id){
          query.id = this.detailData.id
          await nowledgServices.addGroup({...query,type:this.detailData.type_number,pid:this.detailData.id}).then(res => {
            this.groupDialog = false
            this.getGlist()
            this.$emit('saveok')
          })
        }else {
          await nowledgServices.modifyGroup({...query,type:this.detailData.type_number,pid:this.detailData.id}).then(res => {
            this.groupDialog = false
            this.getGlist()
            this.$emit('saveok')
          })
    ...
    async getProgress() {//获取进度
      await nowledgServices.mountSchedule({id:this.detailData.id},(res) => {
    ...
    KnowledgeCommit() {//知识点新增修改
      ...
      try{
        if(!query.id){
          ...
          nowledgServices.addNowledges({...query,type:this.detailData.type_number,pid:this.detailData.id}).then(res=>{
          ...
          nowledgServices.modifyNowledges({...query,type:this.detailData.type_number,pid:this.detailData.id}).then(res=>{
            this.knowledgeDialog = false
            this.getList()
            this.$emit('saveok')
          })
    ...
    knowDel(id=[]){//知识点删除
      ...
      this.$confirm('是否删除选中知识点?').then( res => {
        nowledgServices.deleteNowledges({id,type:this.detailData.type_number,pid:this.detailData.id}).then(res => {
          this.getGlist()
          this.$emit('saveok')
        }).catch(({payload}) => {
          for(let i in payload.messages){
    ...
    removeAnchor(id=[]) {//锚点删除
      ...
      this.$confirm('是否删除选中锚点?').then(res => {
        nowledgServices.delAnchor({id,type:this.detailData.type_number,pid:this.detailData.id}).then(res => {
          this.getAnchorList()
          this.$emit('saveok')
        }).catch(({payload}) => {
          for(let i in payload.messages){
    ...
    async getbookList() {//获取书籍列表
      await nowledgServices.listNowledges({id:this.detailData.id}).then(res => {
    ...
    getBtn() { // 获取可选列表
      nowledgServices.nowledge_btn({id:this.detailData.id}).then( async (res) => {
    ...
    bookinfoBlur(type) {
      if(this.bookInfo.value && !type && this.anchorType == Anchors_chapter){
        this.addAnchorDialog = true
      }else if(this.bookInfo.value && !type && this.anchorType == Anchors_content){
        if(!this.readerId){
    ...
    .left
      ul
        .li(v-for="item in glist" @click="changeGroup(item)" :class="{currGroup:item.id == currGroupId}")
          ...
          .btns(v-if='!readOnly')
            Icon(@click.stop="groupEdit(item)" type="icon-menu-publish")
            Icon(@click.stop="groupDel(item)" type="el-icon-delete")
    .tableWrap
      .nav.clearfix
        ...
        .article
          table.table(v-if="openAnchorList" ref="anchorList")
            ...
            tbody.contentAnchors( )
              tr(v-for="item in anchorList" :key="item.id")
                td
                  .input
