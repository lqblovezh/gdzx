...
import * as services from 'services/x-publish/picture'
...
export default {
  ...
  methods: {
    ...
    getList(
      ...
      pageOffset = (this.page && this.page.offset) || 48,
      query = null
    ) {
      services
        .listPicture({
    ...
    async del(data) {
      return this.services.deletePicture(data)
    },
    openDetailImgDialog(item) {
      //图片放大预览
      this.detailSrc = item.preview_path
                ...
                tbody
                  tr(v-for="item,index in list" :key="item.id")
                    td
                      input(type="checkbox" :value="item.id" name="ids")
                    td.alignL
                      img.cover-photo(:src="item.preview_path")
