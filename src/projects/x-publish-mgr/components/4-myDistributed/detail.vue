...
.div
  ...
  el-tabs( :value="type" @tab-click="handleClick")
              ...
              table.table.border(width="100%")
                thead
                  tr
                    ...
                    th(width=70) 操作
                tbody
                  tr(v-for="item,index in detailData.components" :key="item.id")
    ...
    el-tab-pane( label="扩展属性" name="metadata")
      .meta
        .group.rowWrap( v-for="(item, index) in detailData.attributes"  :key="item.id")
          span( v-text="index+1")
    ...
    el-tab-pane( label="知识点" name="Knowledge")
      .KnowledgeWrap(v-if='gList')
        ul.left
          li(v-for="item in gList" @click="handKnowledgeClick(item)"  :key="item.id") {{item.name}}
        .right
