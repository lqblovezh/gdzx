  ...
  .mainRight
    .content
      ul.navHead.modular(v-if="navList.length")
        li(v-for="item,index in navList" :class="{active:currentIndex==index}" @click="navChange(index,item.id)" :key="item.id")
        ...
        .books.modular(style="padding-bottom:20px")
          ...
          ul.listPanel(v-show="showByImage")
            li.colspan(v-if="!pageList.length")
              .img-box 暂无数据
            li(v-for="item,index in pageList" @click="showDetail(item,'books')" :key="item.id")
              .imgWrap
                .img-box
