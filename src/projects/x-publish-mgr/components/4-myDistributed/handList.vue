  ...
  .boxList
    .books.modular(v-show="pageList.books.length")
      ...
      ul.listPanel(v-show="showByImage")
        li(v-for="item,index in pageList.books" @click="showDetail(item,'books')"  :key="item.id")
          .img-box
      ...
      table.table(width="100%" v-show="!showByImage")
        ...
        tbody
          tr(v-for="item,index in pageList.books")
            ...
            td
              ul.contList
                li(v-for="items in item.contents" v-if="item.contents" :key="item.id")
    ...
    .theme.modular(v-show="pageList.theme.length")
      .title 主题产品
      ul.listPanel(v-show="showByImage")
        li(v-for="item,index in pageList.theme" @click="showDetail(item,'theme')" :key="item.id")
          .img-box.theme
            img(:src="item.img_src")
          .exp
            div
      ...
      table.table(width="100%" v-show="!showByImage")
        thead
          tr
            ...
            th 操作
        tbody
          tr(v-for="item,index in pageList.theme" :key="item.id")
            td.alignL
              img.cover-photo(:src="item.img_src")
              span.bookName(v-text="item.name")
            td(v-text="item.type")
    ...
    .article.modular(v-show="pageList.article&&pageList.article.length")
      .title 推送文章
      ul.listPanel(v-show="showByImage")
        li(v-for="item,index in pageList.article" :key="item.id" @click="showDetail(item,'article')")
          .img-box
            img(:src="item.img_src")
          .exp
            div
      ...
      table.table(width="100%" v-show="!showByImage")
        thead
          tr
            ...
            th 操作
        tbody
          tr(v-for="item,index in pageList.article" :key="item.id")
            td.alignL
              img.cover-photo(:src="item.img_src")
              span.bookName(v-text="item.name")
            td(v-text="item.author")
