...
export default {
  ...
  methods: {
    ...
    push(item) {
      ...
      if (item.version && item.version.length && item.type != 'eCommerce') {
        this.coverDialog = true
      } else {
        this.dispense()
      }
    ...
    coverDispense() {
      ...
      const version_id = this.currentItem.version.find(
        item => item.xread_book_id === this.currentVersion
      ).version_id
