...
export default {
  ...
  methods: {
    ...
    download(downLoadArray, gliBook) {
      let vrDownJson = this.$getStorage('vrDownJson')
      ...
      let files = []
      return gliBook
        .download(downLoadArray, ({ progress, file }) => {
          this.download_percentage = Math.floor(progress * 100)
          if (file) {
            files.push(file)
            vrDownJson[this.currentItem.id] = {
              version: currentVersion,
              files,
            }
            this.$setStorage('vrDownJson', vrDownJson)
          }
        })
        .then(() => true)
    },
    getArList() {
      ...
      if (updateVersion) {
        ...
        if (
          vrDownJson[this.currentItem.id] &&
          vrDownJson[this.currentItem.id].version != currentVersion
        ) {
          this.$MessageBox
            .confirm('是否更新资源')
            .then(res => {
              version = currentVersion
              this.sendRequest(version, true)
            })
            .catch(res => {
              version = vrDownJson[this.currentItem.id].version
    ...
    sendRequest(version, type) {
      this.$service.ar
        .ArList({ id:this.currentItem.id, type: device?device.platform.toLowerCase():'android', version })
        // .ArList({ id: this.currentItem.id, type: 'android', version })
        .then(res => {
          let all_urls = res.data
          if (all_urls.length) {
            this.readAr(all_urls, type)
          } else {
            this.$MessageBox.alert('资源未配置请联系管理员!', '提示')
          }
        })
    },
    readAr(all_urls, type) {
      const index = all_urls[0].lastIndexOf('/')
      const filename = all_urls[0].substr(0, index).split('/files/')[1]
      const gliBook = new GliBookUtil()
      let vrDownJson = this.$getStorage('vrDownJson')
      gliBook
        .isDownloaded(all_urls, vrDownJson[this.currentItem.id])
        .then(({ downLoadArray }) => {
          if (downLoadArray.length) {
            if (type) {
              return this.download(downLoadArray, gliBook)
            }
