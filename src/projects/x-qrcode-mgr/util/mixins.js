...
var main = {
	...
	beforeRouteUpdate (to , from , next){ //路由变化的时候,只需要改变currGroup 然后再查询
	...
	methods: {
		...
		query() {
			var query = formSerialize(document.querySelector("#mainForm"), { hash: true })
		...
		getList( pageNum=this.page&&this.page.num||1 , pageOffset=this.page&&this.page.offset||10, query ={} ){
			...
			this.services.getList({pageNum , pageOffset ,...this.parameter, ...query} ).then( res => {
				this.setList(res.data)
				this.setPage(res.page)
			})
		},
		getGroupList() {//查询分组列表
			return this.services.groupList().then(res => {
		...
		groupChange(id) {  //分组点击事件// 点击后 带参数 转跳路由
			this.$router.push({ path: this.$route.path, query: { id } })
		},
		addGroup(node) {  // 添加分组回调
		...
		showUpShelf(item) {
			if(!item){
				if (!this.$getChecked({data:this.list,name:'set',value:'0',error:'请设置价格后再上架!'})) return
			}
		...
		upShelf(id) { //单个或多个上架
			...
			if (id) {
				...
				ids = this.$getChecked({data:this.list,name:'set',value:'0',error:'请设置价格后再上架!'})
				if (!ids) return
			}
		...
		downShelf(id) {//单个或多个下架
			...
			if (id) {
				...
				ids = this.$getChecked({data:this.list,name:'status',value:'0',error:'未上架的不能下架'});
				if (!ids) {
					return
				}
			}
		...
		delChoosen(id,item) {//删除或多个单个
			...
			if (id) {
				...
				ids = this.$getChecked({data:this.list,name:'product_status_name',value:'使用中',error:'使用中,禁止删除'});
				if (!ids) {
					return
				}
			}
		...
		versionChage(e,item) {
			const oldItem = item
			setVersion({id:e.target.value,type:item.type}).then(res => {
...
const circleMixin = {
	...
	methods: {
		getList( pageNum=this.page&&this.page.num||1 , pageOffset=this.page&&this.page.offset||10, query ={} ){
			this.services.getList({pageNum, pageOffset, id:this.$route.query.id, ...query,status:this.type.value} ).then( res => {
		...
		del(id) {//删除或多个单个
			...
			if (id) {
				...
				ids = this.$getChecked();
				if (!ids) {
					return this.$message.error('错误：未选中相关数据！')
				}
			}
			this.$confirm("是否删除？")
				.then(_ => {
		...
		modify(status,id=[]) {
			...
			this.services.modify({status,id,user_book_circle_id:this.$route.query.pid}).then(res => {
...
export { main, publicMixin, circleMixin, share}
